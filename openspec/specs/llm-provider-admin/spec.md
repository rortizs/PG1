...
Backoffice management of LLM provider credentials for `judgment` and
`triage` review roles: CRUD + activate, encrypted-at-rest storage,
exactly-one-active-per-role assignment, a temporary shared-secret admin
gate, and active-provider resolution that feeds the CAG review pipeline.
The `judgment` role is required for review runs; the `triage` role is
optional and may be backed by DeepSeek for cheap first-pass filtering.
...
after save — only a masked/last-4 representation. Provider role is assigned
at creation time and MUST NOT be changed by PATCH/update operations.
...
#### Scenario: Admin cannot change a provider's role by update

- GIVEN an existing provider config with `role: "judgment"`
- WHEN an admin submits an update containing `role: "triage"`
- THEN the system returns `422 validation_error` naming `role`
- AND the row's stored role remains unchanged
...
#### Scenario: Activating a new triage provider deactivates the previous triage provider only

- GIVEN provider J is active for role `judgment` and provider T1 is active for role `triage`
- WHEN an admin activates provider T2 for role `triage`
- THEN provider T2 becomes the active `triage` provider
- AND provider T1 is deactivated
- AND provider J remains the active `judgment` provider, unaffected
...
and passed; if none exists, the call proceeds without triage credentials and
MUST NOT error.
...
path MUST NOT include the raw key value for any participating provider role
in error messages, logs, `finding` rows, or `audit_event` rows.
...
- GIVEN an active provider's upstream call fails
...
#### Scenario: Triage provider failure does not leak the triage key

- GIVEN an active triage provider fails before or during classification
- WHEN the review run records failure or fail-open metadata
- THEN no response, log output, finding, or audit event contains the raw triage API key
...
model id) served each role that participated in that run. Triage provenance
is present only when a triage provider participated.
