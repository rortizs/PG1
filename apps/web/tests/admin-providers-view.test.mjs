...
test("buildAdminProvidersViewModel lists real provider rows, including role and active/inactive flags", () => {
  const view = buildAdminProvidersViewModel({
    providers: [
      providerRow({ id: 1, role: "judgment", is_active: true }),
      providerRow({ id: 2, role: "triage" }),
    ],
    loadError: null,
  });
  assert.equal(view.kind, "list");
  assert.equal(view.items.length, 2);
  assert.equal(view.items[0].role, "judgment");
  assert.equal(view.items[1].role, "triage");
...
test("maskedKeyLabel never renders anything but the last-four representation", () => {
  assert.equal(
    maskedKeyLabel(providerRow({ api_key_last_four: "9k2p" })),
    "••••9k2p",
  );
});
...
test("buildCreateProviderPayload includes role and the api_key field (required on create)", () => {
  const payload = buildCreateProviderPayload({
    providerName: "deepseek",
    role: "triage",
    modelId: "deepseek-chat",
    apiKey: "sk-deepseek-real-secret-value",
  });
  assert.deepEqual(payload, {
    provider_name: "deepseek",
    role: "triage",
    model_id: "deepseek-chat",
    api_key: "sk-deepseek-real-secret-value",
  });
});

test("buildUpdateProviderPayload omits api_key and role entirely when the key field is blank (role is immutable)", () => {
...
test("buildUpdateProviderPayload includes api_key only when the admin explicitly typed a new one, never role", () => {
  const payload = buildUpdateProviderPayload({
    role: "triage",
    modelId: "claude-sonnet-4-20250514",
...
test("buildActivatePath encodes the provider id into the activate route", () => {
  assert.equal(
    buildActivatePath(42),
    "/api/v1/admin/llm-providers/42/activate",
  );
});
...
test("extractAdminErrorMessage falls back to the server message for other errors, never a silent failure", () => {
  ...
  assert.equal(
    message,
    "provider_name must be one of: claude, deepseek, groq.",
  );
});
