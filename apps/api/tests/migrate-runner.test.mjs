...
test("splitMigration throws an explicit error when markers are missing", async () => {
	...
	assert.throws(() => splitMigration("CREATE TABLE a(id INT);"), /-- UP/);
});

test("listMigrationFiles discovers ALL *.sql files in a directory and sorts them by filename (not a single hardcoded path)", async () => {
...
test("migrate.mjs discovers and applies ALL *.sql files under a migrations directory in filename order, and reverts them in reverse order", async (t) => {
	let pg;
	let migrate;
	try {
...
test("migrate.mjs applies and reverts ALL discovered migrations (baseline + llm_provider_config) in order against live Postgres", async (t) => {
	let pg;
	let migrate;
	try {
		({ default: pg } = await import("pg"));
...
test("migrate.mjs: applying 0001 alone (via an explicit migrationPath) still creates exactly the baseline tables — no regression from auto-discovery", async (t) => {
	let pg;
	let migrate;
	try {
		({ default: pg } = await import("pg"));
